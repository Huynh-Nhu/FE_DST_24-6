...
export default function UpdateFields() {
  ...
  const handleUpdatetModal = () => {
    if (validateUpdate()) {
      ...
      if (isOn) {
        ...
        newPrimaryKey = newPrimaryKey.filter(
          (index) => index !== fieldTempUpdate.id
        );
      }
      ...
      if (isOnforenkey) {
        ...
        const updatedForeignKeys = foreignKeys.filter(
          (key) => key.field_id != fieldTempUpdate.id
        );
  ...
  const handleUpdatetModalNewField = () => {
    if (validateUpdateFieldTemp()) {
      if (!isOn && primaryKey.includes(fieldNew.index)) {
        const newPrimaryKey = primaryKey.filter(
          (index) => index !== fieldNew.index
        );
        setPrimaryKey(newPrimaryKey);
      }
      if (isOn) {
      ...
      if (isOnforenkey) {
        const updatedForeignKeys = foreignKeys.filter(
          (foreignKey) => foreignKey.index !== fieldNew.index
        );
  ...
  useEffect(() => {
    if (
      getTableFields?.foreign_keys?.some(
        (fk) => fk.field_id === fieldTempUpdate.id
      )
    ) {
      setIsOnforenkey(true);
    }
  ...
  const getIdField = (fieldId) => {
    ...
    const foreignKeys = getTableFields.foreign_keys
      ? getTableFields.foreign_keys
      : [];
    const foreignKey = foreignKeys.find((key) => key.field_id == field_id);

    if (foreignKey) {
      const foreignTable = tables.tables?.find(
        (tb) => tb.id == foreignKey.table_id
      );
      if (foreignTable) {
  ...
  const handleUpdatetModalFieldTemp = () => {
    if (!isOn && primaryKey.includes(fieldTempUpdate.index)) {
      const newPrimaryKey = primaryKey.filter(
        (index) => index !== fieldTempUpdate.index
      );
      setPrimaryKey(newPrimaryKey);
    }
    if (isOn) {
    ...
    if (isOnforenkey) {
      const updatedForeignKeys = foreignKeys.filter(
        (foreignKey) => foreignKey.index !== fieldTempUpdate.index
      );
  ...
  const deleteFieldTemp = (fieldId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const tempFieldsUpdate = tempFields.filter(
          (field) => field.index !== fieldId.index
        );
        const newPrimaryKey = primaryKey.filter(
          (index) => index !== fieldId.index
        );
  ...
  useEffect(() => {
    fetch(`${proxy}/db/tables/v/${version_id}`, {
      ...
      .then((resp) => {
        const { success, data, status, content } = resp;
        // //////console.log("data", data)
        console.log("tables", resp);
        if (success) {
          if (data) {
            const filteredData = data.tables.filter(
              (table) => table.id !== parseInt(table_id)
            );
  ...
  const addKey = ({ tableId, data }) => {
    ...
    const matchingItem = data.filter(
      (item) => primaryKey.indexOf(item.index) != -1
    );
    const primaryKeyid = matchingItem.map((item) => item.id);
  ...
  const currentTable = getTableFields.fields?.slice(
    indexOfFirstTable,
    indexOfLastTable
  );
  // //////console.log(currentTable)
  const paginateTable = (pageNumber) => setCurrentPageTable(pageNumber);
  const totalPagesTable = Math.ceil(
    getTableFields.fields?.length / rowsPerPageTable
  );
  /// Add field
  useEffect(() => {
  ...
  const currentFields = tempFields?.slice(
    indexOfFirstFields,
    indexOfLastFields
  );
                  ...
                  <div class="form-group col-lg-6">
                    ...
                    <select
                      onChange={(e) =>
                        setTableFields({
                          ...getTableFields,
                          display_fields: [parseInt(e.target.value)],
                        })
                      }
                      value={getTableFields?.display_fields?.[0]}
                      className="form-control"
                    >
                  ...
                  <div class="col-md-12 col-lg-12">
                    <div class="d-flex align-items-center mb-1">
                      ...
                      {data_pre_import && (
                        <button
                          type="button"
                          class="btn btn-primary custom-buttonadd ml-auto mr-2"
                          onClick={importDataPre}
                        >
                      ...
                      <button
                        type="button"
                        class={`btn btn-primary custom-buttonadd  ${
                          !data_pre_import && "ml-auto"
                        }`}
                        data-toggle="modal"
                      ...
                      {currentTable && currentTable.length > 0 ? (
                        <>
                              ...
                              {currentTable.map((field, index) => (
                                <tr key={field.id}>
                                  ...
                                  <td class="align-center">
                                    {" "}
                                    {primaryKey.includes(field.id) ? (
                                      <img
                                        src="/images/icon/p-key.png"
                                        width={14}
                                        alt="Key"
                                      />
                                    ) : null}
                                  ...
                                  <td>
                                    {formatDate(field?.create_at?.toString())}
                                  </td>
                                  <td
                                    class="align-center"
                          ...
                          <div className="d-flex justify-content-between align-items-center">
                            <p>
                              {lang["show"]} {indexOfFirstTable + 1}-
                              {Math.min(
                                indexOfLastTable,
                                getTableFields.fields?.length
                              )}{" "}
                              {lang["of"]} {getTableFields.fields?.length}{" "}
                              {lang["results"]}
                            </p>
                            <nav aria-label="Page navigation example">
                              <ul className="pagination mb-0">
                                <li
                                  className={`page-item ${
                                    currentPageTable === 1 ? "disabled" : ""
                                  }`}
                                >
                                  <button
                                    className="page-link"
                                    onClick={() =>
                                      paginateTable(currentPageTable - 1)
                                    }
                                  >
                                    &laquo;
                                  </button>
                                </li>
                                {Array(totalPagesTable)
                                  .fill()
                                  .map((_, index) => (
                                    <li
                                      className={`page-item ${
                                        currentPageTable === index + 1
                                          ? "active"
                                          : ""
                                      }`}
                                    >
                                ...
                                <li
                                  className={`page-item ${
                                    currentPageTable === totalPagesTable
                                      ? "disabled"
                                      : ""
                                  }`}
                                >
                ...
                <form>
                  <div class="row">
                    <div class="form-group col-lg-12">
                      ...
                      />
                      {errors.field_name && (
                        <p className="text-danger">{errors.field_name}</p>
                      )}
                    </div>
                    <div class="form-group col-lg-6">
                      <label>
                        {lang["key"]} <span className="red_star">*</span>
                      </label>
                    </div>
                    <div class="form-group col-lg-12 d-flex align-items-center ml-4">
                      ...
                      <input
                        type="checkbox"
                        checked={modalTemp.full_text_search}
                        onChange={({ target: { checked } }) => {
                          setModalTemp({
                            ...modalTemp,
                            full_text_search: checked,
                          });
                        }}
                      />
                    </div>
                    ...
                    <div class="form-group col-lg-12 d-flex align-items-center ml-4">
                      ...
                      <img
                        src={
                          isOn ? "/images/icon/on.png" : "/images/icon/off.png"
                        }
                    ...
                    <div className={`form-group col-lg-6`}>
                      ...
                      {errors.table_id && (
                        <p className="text-danger">{errors.table_id}</p>
                      )}
                    </div>
                    <div className={`form-group col-lg-6`}>
                      <label>
                      ...
                      >
                        ...
                        {fields
                          .filter((field) => {
                            const selectedTableIdAsNumber =
                              Number(selectedTableId);
                            const selectedTable = tables.tables.find(
                              (table) => table.id === selectedTableIdAsNumber
                            );
                            return selectedTable?.primary_key.includes(
                              field.id
                            );
                          })
                          .map((field, index) => {
                      ...
                      {errors.ref_field_id && (
                        <p className="text-danger">{errors.ref_field_id}</p>
                      )}
                    </div>
                    <div class="form-group col-lg-12">
                      <label>
                        {lang["null"]} <span className="red_star">*</span>{" "}
                      </label>
                      <select
                        className="form-control"
                        onChange={(e) =>
                    ...
                    <div class={`form-group col-lg-12`}>
                      ...
                      {errors.DATATYPE && (
                        <p className="text-danger">{errors.DATATYPE}</p>
                      )}
                    </div>
                                  ...
                                  >
                                    ...
                                    <select
                                      ...
                                      value={JSON.stringify(
                                        modalTemp[prop.name]
                                      )} // Sử dụng JSON.stringify để so sánh
                                      onChange={(e) => {
                                        // Phân tích giá trị đã chọn và cập nhật vào trạng thái
                                        const selectedOptionValue = JSON.parse(
                                          e.target.value
                                        );
                                        setModalTemp((prevModalTemp) => ({
                                          ...prevModalTemp,
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        if (
                                          value === "" ||
                                          (/^-?\d+$/.test(value) &&
                                            value >= type.limit.minSize &&
                                            value <= type.limit.maxSize)
                                        ) {
                                          setModalTemp((prevModalTemp) => ({
                                            ...prevModalTemp,
                                            [prop.name]: value,
                ...
                <form>
                  <div class="row">
                    ...
                    <div class="form-group col-lg-12 d-flex align-items-center ml-4">
                      ...
                      <input
                        type="checkbox"
                        checked={fieldTempUpdate.full_text_search}
                        onChange={({ target: { checked } }) => {
                          setFieldTempupdate({
                            ...fieldTempUpdate,
                            full_text_search: checked,
                    ...
                    <div class="form-group col-lg-12 d-flex align-items-center ml-4">
                      ...
                      <input
                        type="checkbox"
                        checked={fieldTempUpdate._index}
                        onChange={({ target: { checked } }) => {
                          setFieldTempupdate({
                            ...fieldTempUpdate,
                            _index: checked,
                        ...
                        {fields &&
                          ...
                          fields
                            ...
                            .map((field, index) => {
                              ...
                              if (
                                foreignKey &&
                                foreignKey.ref_field_id == field.id
                              ) {
                                return (
                                  <option
                                    selected="selected"
                            ...
                            setFieldTempupdate((prevFieldTempUpdate) => {
                              ...
                              if (selectedType.limit) {
                                ...
                                updateValues.MIN =
                                  min !== undefined
                                    ? String(min)
                                    : prevFieldTempUpdate.MIN;
                                updateValues.MAX =
                                  max !== undefined
                                    ? String(max)
                                    : prevFieldTempUpdate.MAX;
                              }
                              if (
                                selectedType.type === "date" ||
                                selectedType.type === "datetime"
                    ...
                    <div class="form-group col-lg-6">
                      <label>
                        {lang["creator"]} <span className="red_star">*</span>
                      </label>
                      <input
                        class="form-control"
                        type="text"
                    ...
                    <div class="form-group col-lg-6">
                      <label>
                        {lang["time"]} <span className="red_star">*</span>
                      </label>
                      <input
                        class="form-control"
                        type="text"
          ...
          <div class="modal-dialog modal-dialog-center">
            <div class="modal-content">
                  ...
                  <div class="row">
                    ...
                    <div class={`form-group col-lg-12`}>
                      <select
                        ...
                        onChange={(e) =>
                          setFieldNew({ ...fieldNew, DATATYPE: e.target.value })
                        }
                      >
                        {types.map((type, index) => (
                          <option key={index} value={type.name}>
                        ...
                        return (
                          <div key={type.id}>
                            {type.props.map((prop, index) => {
                              ...
                              if (inputType === "int") {
                                if (prop.name === "MIN")
                                  defaultValue = fieldNew.MIN;
                                if (prop.name === "MAX")
                                  defaultValue = fieldNew.MAX;
                              }
              ...
              <div class="modal-footer">
                <button
                  type="button"
                  onClick={handleUpdatetModalNewField}
                  class="btn btn-success "
                >
                  {lang["btn.update"]}
                </button>
              ...
              <div class="modal-body">
                <form>
                  <div class="row">
                    <div class="form-group col-lg-12">
                      <label>
                        Tên bảng <span className="red_star">*</span>
                      </label>
                      <input
                        type="text"
                        class="form-control"
                        value={tableUpdate.table_name}
                        onChange={(e) => {
                          setUpdateTable({
                            ...tableUpdate,
                            table_name: e.target.value,
