...
import { METHOD_TYPE } from "./create-api";
...
const UpdateAPI = () => {
  ...
  const [newConditions, setConditions] = useState();
  ...
  const validateApiFieldShow = () => {
    let temp = {};
    temp.fields =
      modalTemp.fields && modalTemp.fields.length > 0 ? "" : lang[""];
  ...
  const handleSubmitModal = () => {
    ...
    if (valid) {
      ...
      // console.log("this is a final payload", modalTemp);
  ...
  useEffect(() => {
    ...
    if (tempFieldParam && Object.keys(tempFieldParam).length > 0) {
      // console.log("this is tempFieldParam",);
      updateApi();
    }
      ...
      .then((resp) => {
        ...
        if (success) {
          if (data) {
            ...
            console.log("filteredAPI", filteredAPI, resp);
    ...
    const fetchData = async () => {
      ...
      for (let tableId of modalTemp.tables) {
        ...
        const filteredFields =
          Array.isArray(tableData.data.fields) && modalTemp?.body
            ? tableData.data.fields.filter((field) =>
                modalTemp.body.includes(field.id)
              )
            : [];
  ...
  const handleSubmitBody = () => {
    ...
    setModalTemp((prevModalTemp) => ({
      ...
      body_update_method: allSelectedFieldBody.map((field_id) => {
        const field = prevModalTemp.body_update_method.find(
          (f) => f.field_id === field_id
        );
        if (field) {
          return field;
        }
        return { field_id, method: METHOD_TYPE.OVERRIDE };
      }),
    }));
  };
  ...
  useEffect(() => {
    ...
    // console.log("this is the selected Table (Thinh)",  tables);
  }, [tables]);
  ...
  useEffect(() => {
    const fetchFields = async (tableId) => {
      ...
      if (resp.success) {
        // console.log("this is the success date (Thinh)",  resp.data);
  ...
  const findTableAndFieldInfo = (fieldId) => {
    for (const [tableId, tableInfo] of Object.entries(tableFields)) {
      ...
      if (fieldInfo) {
        // console.log("This is tableId (Thinh)",tableId);
        // console.log("This is fieldInfo (Thinh)",fieldInfo);
  ...
  const initializeCheckboxState = () => {
    if (modalTemp.body) {
      ...
      // console.log("this is selectedFieldsBody(thinh)", tempBody);
      setSelectedFieldsBody(tempBody);
    }
  };
  // thinh modalTemp

  console.log("modalTemp", modalTemp);
  ...
  function handleChangeModalTempBody({ fieldId, payload }) {
    setModalTemp((prev) => {
      ...
      for (const index in newModalTemp.body_update_method) {
        const item = newModalTemp.body_update_method[index];

        if (item.field_id === fieldId) {
          for (const k in payload) {
            newModalTemp.body_update_method[index][k] = payload[k];
          }
        }
      }
  ...
  const updateFieldStatistical = (sta) => {
    ...
    const raw_group_by = data.filter((field) =>
      sta.group_by.includes(field.fomular_alias)
    );
                ...
                <div class="row column1">
                  <div class="form-group col-lg-6">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setModalTemp({ ...modalTemp, api_name: e.target.value })
                      }
                      placeholder=""
                    />
                    {errorApi.api_name && (
                              ...
                              {tables.map((table, index) => (
                                <tr key={index}>
                                  ...
                                  <td>
                                    <Select
                                      value={(() => {
                                        const type = modalTemp.table_type.find(
                                          ({ table_id }) =>
                                            table_id === table.id
                                        );
                                        return {
                                      ...
                                      onChange={({ value }) => {
                                        setModalTemp((prev) => {
                                          ...
                                          if (i > -1) {
                                            newData.table_type[i].type = value;
                                          } else {
                                            const initials = {
                                              table_id: table.id,
                                              type: value,
                                            ...
                                            if (newData?.table_type?.length) {
                                              newData.table_type.push(initials);
                                            } else {
                        ...
                        <div class="col-md-12 col-lg-12 bordered">
                          <div class="d-flex align-items-center mb-1">
                            ...
                            <button
                              type="button"
                              class="btn btn-primary custom-buttonadd ml-auto"
                              onClick={initializeCheckboxStateParam}
                              data-toggle="modal"
                        ...
                        <div class="col-md-12 col-lg-12 bordered">
                          <div class="d-flex align-items-center mb-1">
                            ...
                            <button
                              type="button"
                              class="btn btn-primary custom-buttonadd ml-auto"
                              onClick={initializeCheckboxState}
                              data-toggle="modal"
                            ...
                            {modalTemp && modalTemp.body.length > 0 ? (
                              <>
                                <table class="table table-striped">
                                  ...
                                  <tbody>
                                    {modalTemp.body_update_method.map(
                                      (
                                        {
                                          field_id: fieldId,
                                          method: method_type,
                                        ...
                                        return (
                                          <tr key={`${tableId}-${fieldId}`}>
                                            ...
                                            <td>{tableInfo.table_name}</td>
                                            {/* thinh Đây là danh sắp các trường dữ liệu */}
                                            <td>
                                              <select
                                                class="form-select"
                        ...
                        <div class="col-md-12 col-lg-12 bordered">
                          <div class="d-flex align-items-center mb-1">
                            ...
                            <button
                              type="button"
                              class="btn btn-primary custom-buttonadd ml-auto"
                              onClick={initializeCheckboxStateShow}
                              data-toggle="modal"
                                  ...
                                  <div key={fieldIndex}>
                                    <label>
                                      <input
                                        ...
                                        type="checkbox"
                                        checked={
                                          selectedFieldsBody[tableId]?.includes(
                                            field.id
                                          ) ?? false
                                        }
                                        onChange={(e) => {
            ...
            <div class="modal-content">
              ...
              <div class="modal-footer">
                ...
                <button
                  type="button"
                  data-dismiss="modal"
                  id="closeModalExternalBodyUpdate"
                  class="btn btn-danger"
                >
                  {lang["btn.close"]}
                </button>
