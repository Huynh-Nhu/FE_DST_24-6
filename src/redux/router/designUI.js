...
const changeDataByPath = (pages, json, path) => {
  if (path.length == 1) {
    ...
    if (pages[path[0]]) {
      pages[path[0]].children = changeDataByPath(
        pages[path[0]].children,
        json,
...
const removePageByPath = (pages, path) => {
  if (path.length == 1) {
    ...
    pages[path[0]].children = removePageByPath(
      pages[path[0]].children,
      path.slice(1, path.length)
    );
    return pages;
  ...
  if (page) {
    ...
    if (path.length > 0) {
      if (path.length == 1) {
        ...
        const newPages = changeDataByPath(
          cleasedPages,
          parent,
          path.slice(0, path.length - 1)
        );
...
const updateChildComponent = (components, target_id, values) => {
  ...
  for (let i = 0; i < components.length; i++) {
    ...
    if (id == target_id) {
      ...
      if (children) {
        components[i].children = updateChildComponent(
...
const modifyChildrenRecursive = (components, target_id, newChildren) => {
  ...
  for (let i = 0; i < components.length; i++) {
    ...
    if (id == target_id) {
      ...
      if (children) {
        components[i].children = modifyChildrenRecursive(
          components[i].children,
          target_id,
          newChildren
        );
...
const removeChildComponent = (components, target_id) => {
  ...
  for (let i = 0; i < components.length; i++) {
    ...
    if (id == target_id) {
      ...
      if (children && !found) {
        components[i].children = removeChildComponent(
          components[i].children,
          target_id
        );
      }
  ...
  if (target) {
    ...
    for (let i = 0; i < components.length; i++) {
      if (components[i].children) {
        components[i].children = insertChildComponent(
          components[i].children,
          components[i].id,
          target_id,
          position,
...
const insertComponent = (state, action) => {
  ...
  if (block) {
    ...
    floating.block = undefined;
    state.page.component = insertChildComponent(
...
const addChildToComponent = (components, target_id, block, col) => {
  ...
  for (let i = 0; i < components.length; i++) {
    ...
    if (id == target_id) {
      ...
      if (children) {
        components[i].children = addChildToComponent(
          components[i].children,
          target_id,
          block
        );
...
const overrideComponent = (components, target_id, component) => {
  ...
  for (let i = 0; i < components.length; i++) {
    ...
    if (id == target_id) {
      ...
      if (children) {
        components[i].children = overrideComponent(
          components[i].children,
          target_id,
          component
        );
...
const mappingError = (target, source, isFalsy = false) => {
  ...
  // Ehhh, This one is a bit redundant
  return target;
};

const handleError = (state, action) => {
  const newError = state.cache.error;
  const { payload = {} } = action;

  mappingError(newError, payload);

  state.cache.error = newError;
  return state;
};
